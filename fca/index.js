...
function buildAPI(globalOptions, html, token, jar) {
  ...
  log.info("[ DEKU LOGIN ] -", `Logged in as ${userID}`);
  ...
  if (oldFBMQTTMatch) {
    ...
    log.info("[ DEKU LOGIN ] -", `Got this account's message region: ${region}`);
  } else {
    ...
    if (newFBMQTTMatch) {
      ...
      if (legacyFBMQTTMatch) {
        ...
        region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
        log.warn("[ DEKU LOGIN ] -", `Cannot get sequence ID with new RegExp. Fallback to old RegExp (without seqID)...`);
        log.info("[ DEKU LOGIN ] -", `Got this account's message region: ${region}`);
        log.info("[ DEKU LOGIN ] -", `[Unused] Polling endpoint: ${legacyFBMQTTMatch[6].split('",')[0]}`);
      } else {
        log.warn("[ DEKU LOGIN ] -", "Cannot get MQTT region & sequence ID.");
        api.htmlData = html;
      }
    }
  }
                    ...
                    utils
                      ...
                      .then(function (res) {
                        try {
                          ...
                          log.info('[ DEKU LOGIN ] -', 'Verified from browser. Logging in...');
                          var appState = utils.getAppState(jar);
                          return loginHelper(appState, email, password, loginOptions, cb);
                        }
                      })
                      .catch(function (err) {
                        log.error('[ DEKU LOGIN ] -', err);
                        return cb(err);                        
                      });
                  }
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function () {
      log.info("[ DEKU LOGIN ] -", 'Done logging in.');
